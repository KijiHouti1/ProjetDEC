...
#############--FONCTIONS--##########################
def init():         #initialisation des IO et du SPI
...
def write_msg(msg):         #ecriture sur le bus spi
    for current in range(len(msg)):
        print msg[current]
        readDummy = spi.xfer2(msg[current])
	
def receive():              #lecture sur le bus spi
    global readData
...
def key_locking(master):        #fonction qui gere le lock entre les pi et le pic pour determiner ce que la communication doit faire
...
def action():                       #fonction qui gere les selections par la souris
    global mode_ecran
    global fond
    global selection1
    global selection2
    ...
    if mode_ecran == 1:                                 #actions effectuees a l'ecran jeu
        ...
        if (position == 26):
            mode_ecran = 5  #reception
            you_got_mail = False

        if (position >= 55 and position <= 64):     #selection de la couleur dans la rangee du haut
            game_data = (position -55)
            print game_data
        elif (position >= 81 and position <= 90):   #selection de la couleur dans la rangee du bas
            game_data = (position -71)
            print game_data
        else:
            pass
        ...
        if position == 71:              #selection joueur 1
            if selection1 == black:
                ...
                selection4 = black
                dest = 0xA1
            elif selection1 == white:   #deselection
                selection1 = black
                selection2 = black
                selection3 = black
                selection4 = black
            ...
            if selection2 == black:
                ...
                selection4 = black
                dest = 0xA2
            elif selection2 == white:   #deselection
                selection1 = black
                selection2 = black
                selection3 = black
                selection4 = black
            ...
            if selection3 == black:
                ...
                selection4 = black
                dest = 0xA3
            elif selection3 == white:   #deselection
                selection1 = black
                selection2 = black
                selection3 = black
                selection4 = black
            ...
            if selection4 == black:
                ...
                selection4 = white
                dest = 0xA4
            elif selection4 == white:   #deselection
                selection1 = black
                selection2 = black
                selection3 = black
                selection4 = black
        ...
        if (position >= 181 and position <= 182):               #go -> send message       
            ...
            msg_to_send.append((source))
            msg_to_send.append((dest))
            msg_to_send.append((msg_type))
	    msg_to_send.append((len(game_data)))
    ...
    if mode_ecran == 2:             #messagerie
        if (position >= 0 and position <= 4):
            mode_ecran = 0  #menu principal

        if (position >= 27 and position <= 31):     #clavier
            mode_ecran = 4

        if (position >= 19 and position <= 26):     #reception
            mode_ecran = 5

        if (position >= 157 and position <= 161):   #effacer
            Message = ""
            
        

        if position == 76:              #joueur 1
            if selection5 == black:
                selection5 = white
                selection6 = black
                selection3 = black
                selection4 = black
                dest = 0xA1
            elif selection5 == white:   #deselection
                selection5 = black
                selection6 = black
                selection7 = black
                selection8 = black
                dest = None
        elif position == 102:           #joueur 2
            if selection6 == black:
                selection5 = black
                selection6 = white
                selection7 = black
                selection8 = black
                dest = 0xA2
            elif selection6 == white:   #deselection
                selection5 = black
                selection6 = black
                selection7 = black
                selection8 = black
                dest = None
        elif position == 128:           #joueur 3
            if selection7 == black:
                selection5 = black
                selection6 = black
                selection7 = white
                selection8 = black
                dest = 0xA3
            elif selection7 == white:   #deselection
                selection5 = black
                selection6 = black
                selection7 = black
                selection8 = black
                dest = None
        elif position == 154:           #joueur 4
            if selection8 == black:
                selection5 = black
                selection6 = black
                selection7 = black
                selection8 = white
                dest = 0xA4
            elif selection8 == white:   #deselection
                selection5 = black
                selection6 = black
                selection7 = black
                selection8 = black
        ...
        if (position >= 181 and position <= 182):       #go pour envoyer le message
            msg_to_send = []                #message que l'on envoie
            msg_type = 0xEE                 #textdata
            msg_to_send.append([source])
            msg_to_send.append([dest])
            msg_to_send.append([msg_type])
            msg_to_send.append([len(Message)])

            for i in range(0,(len(Message))):
                msg_to_send.append([ord(Message[i])])
	    envoi = True
            print msg_to_send
    ...
    if mode_ecran == 4:             #clavier pour ecrire message
        clavier()
    ...
    if mode_ecran == 5:
        ...
        if (position >= 157 and position <= 163):       #precedent message
            if message_affiche == 0:
                message_affiche = len(boite_reception)
            else:
                message_affiche -= 1

        if (position >= 177 and position <= 182):       #suivant pour prochain message
            if message_affiche == len(boite_reception):
                message_affiche = 0
            else:
                message_affiche += 1
    ...
    return mode_ecran,envoi, fond, selection1, selection2, selection3, selection4,selection5, selection6, selection7, selection8, Message, dest, msg_type, position, message_affiche
...
def affichage():
    ...
    joueurs = {0xa1 : '1',0xa2 : '2',0xa3 : '3',0xa4 : '4'}
        
    if mode_ecran == 0:         #menu principal
        screen.fill(fond)

        if you_got_mail == True:
            notif = fontnotif.render("!", True, white)
            screen.blit(notif,[500,5])
    ...
    if mode_ecran == 2:         #messagerie
        ...
        textreception = font.render("Reception", True, white)
        ...
        screen.blit(textreception,[450,0])
    ...
    if mode_ecran == 3:             #parametres
        ...
        if you_got_mail == True:
            notif = fontnotif.render(textjeu, True, white)
            screen.blit(notif,[500,5])
    ...
    if mode_ecran == 5:
        ...
        if len(boite_reception) > 1 :
            precedent = font.render("Precedent", True, white)
            suivant = font.render("Suivant", True, white)

            screen.blit(precedent,[1,255])
            screen.blit(suivant,[450,255])

        pygame.draw.rect(screen, white, [5, 70, 440, 180])
        joueur_source = "Envoye par le joueur " + joueurs[boite_reception[message_affiche][0]]
        textqui = font.render(joueur_source,True,white)
        textreception1 = fontmessage.render(boite_reception[message_affiche][4:27], True, black)
        textreception2 = fontmessage.render(boite_reception[message_affiche][27:54], True, black)
        textreception3 = fontmessage.render(boite_reception[message_affiche][54:81], True, black)
        textreception4 = fontmessage.render(boite_reception[message_affiche][81:108], True, black)
        textreception5 = fontmessage.render(boite_reception[message_affiche][108:135], True, black)
        textreception6 = fontmessage.render(boite_reception[message_affiche][135:162], True, black)
        textreception7 = fontmessage.render(boite_reception[message_affiche][162:189], True, black)
        textreception8 = fontmessage.render(boite_reception[message_affiche][189:216], True, black)
        textreception9 = fontmessage.render(boite_reception[message_affiche][216:240], True, black)

        screen.blit(textqui,[1,40])
        screen.blit(textreception1,[6,71])
        screen.blit(textreception2,[6,91])
        screen.blit(textreception3,[6,111])
        screen.blit(textreception4,[6,131])
        screen.blit(textreception5,[6,151])
        screen.blit(textreception6,[6,171])
        screen.blit(textreception7,[6,191])
        screen.blit(textreception8,[6,211])
        screen.blit(textreception9,[6,231])

    return Message, message_affiche

def clavier():              #cree une nouvelle image de clavier
    global donekb
    global tabposkb
    global couleur
    global shift
    ...
    screen = pygame.display.set_mode(size, pygame.FULLSCREEN)#, pygame.FULLSCREEN)
...
global spi_actif
spi_actif = False
global you_got_mail         #variables et listes pour la reception
global boite_reception
boite_reception = []
you_got_mail = False
MY_ADDR = 0xA1
...
fontnotif = pygame.font.Font(None, 30)
    ...
    if(GPIO.input(22) == True):
        ...
        if msg_status == True:
            if pic_is[0] == 0xa6:
                ...
                if Msg[2] == 0xEE:
                    you_got_mail = True
                    boite_reception.append(Msg)
                elif Msg[2] == 0xDD:
                    pass    #ajouter le reception de game data
            if pic_is[0] == 0x06:
